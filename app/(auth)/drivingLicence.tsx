...
const DrivingLicence = () => {
  ...
  const uploadImageToCloudinary = async (imageUri: string) => {
    ...
    try {
      const res = await fetch("https://api.cloudinary.com/v1_1/dfswndziq/image/upload", {
        method: "POST",
        body: formData,
      });
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const imageUrl = licenceImage.startsWith("http")
        ? licenceImage
        : await uploadImageToCloudinary(licenceImage);
      if (!imageUrl) {
        setLoading(false);
